...
## 미해결 이슈
- **[백엔드 대기]** CORS OPTIONS 403 재발 — Spring Security가 preflight 차단, 백엔드 수정 요청 완료 ([상세](./project_cors_current_issue.md)) / Vercel `VITE_API_BASE_URL` 변경 + 재배포도 남음
...
- [Notion TIL 페이지](./reference_notion_til.md) — `/update-til` 커맨드 + 매일 18:30 KST 자동 실행 (trig_01AhUdgHMLPiEwTuYBRgtExv)
