...
**이벤트 4종 삽입은 백엔드 의존성 없이 즉시 착수 가능.**
- 이벤트는 익명(GA4 client_id only)으로 발송
- `first_post_created` 판별: `/me.postCount === 0` 프론트 단독 처리
...
- 이벤트 4종 삽입 작업은 바로 착수 가능. `jira_draft.md` 상단 [업로드 대기] 스토리 참고.
- Clarity `identify()` 호출 코드 추가 금지.
