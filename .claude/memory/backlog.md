...
- [-] **D-05** 치료영역 배지(인증 치료사 닉네임 옆) — **보류 (인증 로직 완성 대기)**. ⚠️ "바로 가능" 아님 — 치료사 인증 PENDING 과도기라 배지 미노출 용인, 인증 로직 완성 후 재검토(`project_auth_policy_mvp_immediate_approval.md`). 함정: `post.therapyArea`는 작성자 전문분야가 아니라 **고민카드 글 카테고리** 필드(닉네임 옆 배지 데이터 아님). 2026-06-19 재추천했다 잡힘 — 재추천 금지
