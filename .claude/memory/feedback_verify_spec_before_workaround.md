...
## 도메인 확장 사례 — 라이브러리/도구 선택 (2026-04-27)

같은 원칙이 "라이브러리/도구 선택" 도메인에도 적용된다는 것이 확인됨.

**케이스:** SEO 옵션 2 진행 중 `vite-react-ssg` 채택 직전. 평판(High, Benchmark 86)·스니펫 수(113)·이름 매칭만 보고 설치하려던 시점에 공식 README를 먼저 확인 → "React Router v7 사용자에게는 비권장" 명시 발견 → `vite-plugin-react-ssg`로 갈아탐.

**막은 비용:** 라우트 16개를 `createBrowserRouter` 객체 config로 전환한 뒤에야 호환성 문제 발견 → 롤백 + 재마이그레이션. 추정 3~5시간 손실 회피.

**확장된 적용 규칙:**

- 새 라이브러리/플러그인 설치 직전, **공식 문서(README/Getting Started)에서 현재 스택 호환을 명시적으로 확인**
- 평판·인기 점수·스니펫 수는 **출발점일 뿐 결정 근거가 아님** — 호환 명시가 우선
- 호환 명시가 없거나 모호하면 → "맞다고 가정"하지 말고 GitHub Issues / package.json `peerDependencies`로 추가 검증
- 설치(=`npm install`) 직전이 검증 마지막 게이트. 그 이후엔 롤백 비용이 코드 변경량과 함께 커짐
