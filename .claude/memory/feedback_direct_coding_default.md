...
## 작업 성격에 따른 unlock 기준 (2026-05-14 보강)

데드라인 임박 외에 **작업 성격**도 unlock 정당화 축에 추가합니다.

| 작업 성격 | unlock 기준 |
|---|---|
| 알고리즘·구조 설계, 새 기능 구현, 새 로직 도입 | 데드라인 임박 시에만 (학습 우선) |
| **텍스트 치환·일괄 변경 등 보일러플레이트** | **메커니즘 이해 완료 후엔 unlock OK** |
| 인프라/툴링 코드 | 명시 허락 시 (위 예외 룰) |

**Why:** 학습 가치는 "메커니즘 이해"에서 나오고, 그 이해가 끝난 후의 적용 단계가 단순 텍스트 치환이면 학습 추가 가치 없이 시간만 소모. 2026-05-14 SEO 학습 1-3단계(검색엔진 3단계 모델 / SPA 한계 / 레버 카탈로그) 끝낸 후 16 파일 27 라인 브랜드 표기 일괄 치환에서 사용자가 unlock 정당화하며 박힌 패턴.

**How to apply:** 코드 변경 요청 받았을 때:
1. 변경이 단순 치환·반복 패턴인가? → unlock OK 후보 (사용자에게 옵션 제시 후 본인 결정)
2. 변경이 새 로직·구조인가? → 기본 룰(데드라인 임박 시에만)
3. 사용자가 학습 단계를 명시적으로 끝냈는가? → unlock 후 일괄 진행 가능
4. 사용자가 학습 우선이라고 명시한 경우엔 보일러플레이트도 가이드만 (이 룰은 사용자 의사 우선)
