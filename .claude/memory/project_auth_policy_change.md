...
## 닉네임 정책 변경 (2026-04-02)
- 가입 시 닉네임 입력 제거 → 랜덤 자동 부여 → 나중에 수동 수정
- 현재: 백엔드 nickname required라서 프론트에서 `치료사${랜덤6자리}` 임시 전송 (api/auth.ts)
- 백엔드 nickname 필수 해제 시 프론트 임시 코드 삭제 필요

## 게시글 title 필드 제거 (2026-04-02)
- 프론트 title input 제거, API에 `title: ""` 빈 문자열 임시 전송
- 백엔드 title optional/삭제 요청 대기 중 (❌ P0 잔존)
