...
백엔드 로그인 응답이 스펙과 달라 프론트 로그인이 작동하지 않음 (2026-03-15 확인).
프론트 타입은 확정된 응답 포맷에 맞게 수정 완료 (2026-03-15).

**확정된 응답 포맷 (백엔드 구현 대기 중):**
```http
HTTP/1.1 200 OK
...
**How to apply:** 백엔드 수정 완료 전까지 실제 백엔드로 로그인 불가. MSW 모드로만 개발 가능.
수정 완료되면: 로그인 테스트 → 콘솔 스크립트로 게시글 19개 일괄 삽입 → 전체 기능 테스트
