...
| 3. 구현 | 45~60분 | 빈 파일에 AI 없이 직접 작성. 최대한 스스로 생각하되, 정말 모르면 원본 참고 OK |
| 3-1. 반복 | 15~30분 | 구현 중 원본을 봤다면, 원본 안 보고 완성될 때까지 2~3회 반복 작성 |
...
- ✅ 기존 프로젝트 다른 파일 참고 OK
- ✅ 막히면 원본 코드 참고 OK — 단, 봤으면 반복 작성 필수
- ❌ AI에게 묻기 금지
- ❌ 구글에서 "react OO 예제" 복붙 금지

### 반복 작성 규칙 (3-1단계)

- 구현 중 원본을 한 번이라도 봤으면, 원본 안 보고 완성될 때까지 2~3회 반복
- 최대 5회까지. 5회 안에 못 하면 그것도 기록 (진짜 약점)
- 원본 안 보고 처음부터 끝까지 완성하면 종료
